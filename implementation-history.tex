...
The history.py script creates the schema information tables and then it populates them with information about the slave database schema.
...
Marty starts by inspecting the schema of the slave database before any WAL records have been applied to it.
When this is done the first version of the database schema has been saved in the history database.
Marty then starts the WAL replay in the slave database and reads the replay log.

When a new schema is created the replay log contains an insert heap record for the pg\_namespace table.
Marty queries the slave about this new schema and creates a new history version that includes it.
...
If a schema, table or column is altered, e.g. when they are renamed, the WAL has a heap update record.
Similarly when a schema, table or column is dropped the WAL has a heap delete record.
